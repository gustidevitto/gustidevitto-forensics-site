...
import { ArrowRight, ChevronLeft, ChevronRight, Lock, Activity, CheckCircle2, AlertTriangle, XCircle, FileBarChart2 } from 'lucide-react'
import { NeuralMeshBackground } from '@/components/ui/neural-mesh'
import { useTranslation } from 'react-i18next'
import { Button } from '@/components/ui/button'
import { calculateMarginAudit } from '@/lib/fip-engine'
import type { MarginAuditInputs, MarginAuditResult } from '@/types/fip-lite'
import { submitLead } from '@/lib/googleSheetsAPI'

export const Route = createFileRoute('/margin-audit')({
    component: MarginAuditDiagnostic,
})

function MarginAuditDiagnostic() {
    const { t } = useTranslation()
    
    // State
    const [step, setStep] = useState<number>(1)
    const [isCalculating, setIsCalculating] = useState(false)
    const [result, setResult] = useState<MarginAuditResult | null>(null)
    const [isUnlocked, setIsUnlocked] = useState(false)
    const [isSubmitting, setIsSubmitting] = useState(false)
    
    const [inputs, setInputs] = useState<MarginAuditInputs>({
        revenue: 0,
        idealCogs: 0,
        actualCogs: 0,
        labor: 0,
    ...
    const handleRunDiagnostic = () => {
        ...
        setTimeout(() => {
            const data = calculateMarginAudit(inputs)
            setResult(data)
    ...
    const handleUnlock = async (e: React.FormEvent) => {
        ...
        await submitLead({
            ...leadData,
            phone: '',
            source: 'Autopsy - Margin',
            efficiencyVerdict: result?.layer2.efficiencyVerdict
        })
    ...
    const renderInputField = (name: keyof MarginAuditInputs, labelKey: string, hintKey: string, isPercent = false) => (
        <div className="space-y-2">
            ...
            <div className="relative">
                {!isPercent && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40">$</span>}
                <input 
                    type="text"
                    name={name}
                    value={inputs[name] === 0 ? '' : inputs[name].toLocaleString()}
                    onChange={handleChange}
                    className={`w-full bg-white/5 border border-white/10 p-4 ${isPercent ? 'px-4' : 'pl-8'} font-mono text-xl focus:border-white focus:outline-none transition-colors rounded-none`}
                    placeholder="0"
                />
            </div>
            <p className="text-xs text-white/40 font-light">{t(`margin_audit.form.${hintKey}`, hintKey)}</p>
        </div>
    )
                        ...
                        >
                            {step === 1 && (
                                <>
                                    ...
                                    {renderInputField("idealCogs", "ideal_cogs.label", "ideal_cogs.hint")}
                                    {renderInputField("actualCogs", "actual_cogs.label", "actual_cogs.hint")}
                                </>
                            )}
                            ...
                            {step === 2 && (
                                <>
                                    ...
                                    {renderInputField("workingHours", "working_hours.label", "working_hours.hint", true)}
                                    {renderInputField("headcount", "headcount.label", "headcount.hint", true)}
                                </>
                            )}
                    ...
                    <div className="flex justify-between items-center mt-12 pt-8 border-t border-white/10">
                        ...
                        {step < 3 ? (
                            ...
                            <Button 
                                ...
                                className="rounded-none bg-amber-600 hover:bg-amber-700 text-white uppercase tracking-widest font-black px-8"
                            >
            ...
            <div className="max-w-4xl mx-auto space-y-12 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-8 border border-white/10 bg-white/[0.02] backdrop-blur relative overflow-hidden">
                        ...
                        <div className="text-5xl font-mono font-black mb-2 flex items-end gap-2 text-amber-500">
                            {result.layer1.gpLeakagePercent.toFixed(1)} <span className="text-2xl mb-1">%</span>
                        </div>
                        <p className="text-white/40 text-sm font-light uppercase tracking-widest border-t border-white/10 pt-4 mt-4">
                            Est. Loss: <span className="text-white px-2">{formatCurrency(result.layer2.leakageValue.min)} - {formatCurrency(result.layer2.leakageValue.max)}/mo</span> 
                        </p>
                    </div>
                ...
                <div className={`p-8 border ${result.layer2.efficiencyVerdict === 'critical' ? 'border-red-500/50 bg-red-500/5' : 'border-white/10 bg-white/5'} flex items-start`}>
                    ...
                    <div className="border border-white/10 p-1 bg-[url('/noise.png')] relative overflow-hidden">
                        ...
                        <div className="p-8 pb-12 text-center relative z-10 flex flex-col items-center">
                            ...
                            <p className="text-white/60 mb-8 max-w-lg mx-auto font-light">
                                {t('margin_audit.gatekeeper.desc', 'Enter your email to reveal exactly where your margin is leaking and the locked vectors.')}
                            </p>
                            ...
                            <form onSubmit={handleUnlock} className="w-full max-w-md space-y-4">
                                ...
                                >
                                    {isSubmitting ? 'Authenticating...' : t('margin_audit.gatekeeper.btn', 'Unlock Report')}
                                </Button>
                    ...
                    >
                        ...
                        <div className="mt-16 pt-16 border-t border-white/10 border-dashed text-center">
                            ...
                            <div className="flex flex-col sm:flex-row justify-center gap-4">
                                ...
                                <Link to="/growth-scan" className="px-6 py-4 border border-white/20 hover:bg-white/5 transition-colors uppercase tracking-widest text-xs font-black inline-flex items-center justify-center">
